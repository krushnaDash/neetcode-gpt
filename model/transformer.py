...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.ln1 = nn.LayerNorm(model_dim) # Layer normalisation
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        x = embedded
        normalized_x=self.ln1(x) # Step 1 normalise
        x = x + self.attention(normalized_x)  # Step 2 normalize input to attention, add to original x
        normalized_x=self.ln2(x) # Step 3 normalise
        x = x + self.nn(normalized_x)         # normalize input to ffn, add to original x
